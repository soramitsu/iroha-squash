...
fn collect_token_definitions(
    wsv: &WorldStateView,
) -> impl ParallelIterator<Item = Instruction> + '_ {
    register! {
        map_values!(wsv.permission_token_definitions())
    }
}

fn collect_roles(wsv: &WorldStateView) -> impl ParallelIterator<Item = Instruction> + '_ {
...
fn collect_accounts(domain: Domain) -> impl ParallelIterator<Item = Instruction> {
    register! {
...
fn collect_asset_definitions(domain: Domain) -> impl ParallelIterator<Item = Instruction> {
    register! {
...
fn collect_nfts(
    ...
    nfts.into_par_iter()
        ...
        .chain(transfers.into_par_iter())
}

fn collect_triggers(wsv: &WorldStateView) -> impl ParallelIterator<Item = Instruction> + '_ {
    register! {
      wsv.triggers().ids().into_par_iter().map(|id| {
...
fn collect_domains(wsv: &WorldStateView) -> impl ParallelIterator<Item = Instruction> + '_ {
    register! {
