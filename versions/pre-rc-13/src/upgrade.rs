...
impl Upgrade for from_crypto::Hash {
    ...
    fn upgrade(self) -> Self::To {
        if self == Self::zeroed() {
            panic!("Encountered a zeroed hash")
        } else {
...
impl_upgrade! {
    ...
    |from: From| {
        match from {
            From::MaximumFaultyPeersAmount(v) => format!("?MaximumFaultyPeersAmount={v}").parse().unwrap(),
            From::BlockTime(v) => format!("?BlockTime={v}").parse().unwrap(),
            From::CommitTime(v) => format!("?CommitTime={v}").parse().unwrap(),
            From::TransactionReceiptTime(v) => format!("?TransactionReceiptTime={v}").parse().unwrap(),
        }
    }
}
...
impl_upgrade! {
    ...
    |from: From| {
        To {
            ...
            height: from.height,
            previous_block_hash: Some(from.previous_block_hash.upgrade()),
            transactions_hash: Some(from.transactions_hash.upgrade().typed()),
            rejected_transactions_hash: Some(from.rejected_transactions_hash.upgrade().typed()),
...
impl Upgrade for from::events::data::prelude::DomainEvent {
    ...
    fn upgrade(self) -> Self::To {
        match self {
            ...
            Self::Created(v) => Self::To::Created(to::domain::Domain::new(v.upgrade())),
...
impl Upgrade for from::events::data::prelude::AssetDefinitionEvent {
    ...
    fn upgrade(self) -> Self::To {
        match self {
            Self::Created(v) => Self::To::Created(to::asset::AssetDefinition::new(
                v.upgrade(),
                iroha_core::tx::AssetValueType::Quantity,
            )),
...
impl Upgrade for from::events::data::prelude::RoleEvent {
    ...
    fn upgrade(self) -> Self::To {
        match self {
            Self::Created(v) => Self::To::Created(to::role::Role::new(v.upgrade())),
            Self::Deleted(v) => Self::To::Deleted(v.upgrade()),
...
impl Upgrade for from::events::data::prelude::AssetEvent {
    ...
    fn upgrade(self) -> Self::To {
        match self {
            Self::Created(v) => Self::To::Created(to::asset::Asset::new(
                v.upgrade(),
                to::asset::AssetValue::Store(Default::default()),
            )),
            Self::Deleted(v) => Self::To::Deleted(v.upgrade()),
...
impl Upgrade for from::events::data::prelude::AccountEvent {
    ...
    fn upgrade(self) -> Self::To {
        match self {
            ...
            Self::Created(v) => Self::To::Created(to::account::Account::new(v.upgrade(), vec![])),
            Self::Deleted(v) => Self::To::Deleted(v.upgrade()),
...
impl_upgrade! {
    ...
    |from: From| {
        match from {
            ...
            From::Parameter(v) => To::Identifiable(to::IdentifiableBox::Parameter(Box::new(v.upgrade()))),
