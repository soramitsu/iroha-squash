...
fn collect_token_definitions(wsv: &WorldStateView) -> impl Iterator<Item = InstructionBox> + '_ {
...
fn squash(path: &str) -> anyhow::Result<String> {
    ...
    let genesis = RawGenesisBlock {
        transactions,
        validator: ValidatorMode::Path(ValidatorPath {
            validator_path: "./test.wasm".parse().unwrap(),
        }),
    };
...
pub extern "C" fn upgrade(from: *const libc::c_char) -> *mut libc::c_char {
    ...
    let genesis = RawGenesisBlock {
        ...
        validator: ValidatorMode::Path(ValidatorPath {
            validator_path: "/needs_rebuild.wasm".parse().unwrap(),
        }),
    };
